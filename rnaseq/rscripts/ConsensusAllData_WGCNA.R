...
exp2 <- list(rfdata = list(data = as.data.frame(t(rf_norm))),
                  rmdata = list(data = as.data.frame(t(rm_norm))))
...
moduleMembership <- mtd.mapply(bicorAndPvalue, multiExpr, consensusMEs, 
                               MoreArgs = list(alternative = "two.sided", use = "pairwise.complete.obs", 
                                               maxPOutliers = 0.1))
MM_female <- as.data.frame(moduleMembership$wtfdata$data$bicor)
...
MM_cfemale$Probe <- rownames(MM_cfemale)
MM_cfemale$Module <- consensusMods$colors
write.table(MM_cfemale, "Consensus Modules FEMALES CLAMS Module Membership.txt", sep = "\t", quote = FALSE, row.names = FALSE)

MM_cmale <- as.data.frame(moduleMembership$cmdata$data$bicor)
colnames(MM_cmale) <- gsub(pattern = "ME", replacement = "", x = colnames(MM_cmale), fixed = TRUE)
MM_cmale$Probe <- rownames(MM_cmale)
MM_cmale$Module <- consensusMods$colors
write.table(MM_cmale, "Consensus Modules MALES CLAMS Module Membership.txt", sep = "\t", quote = FALSE, row.names = FALSE)

MM_rfemale <- as.data.frame(moduleMembership$rfdata$data$bicor)
colnames(MM_rfemale) <- gsub(pattern = "ME", replacement = "", x = colnames(MM_rfemale), fixed = TRUE)
MM_rfemale$Probe <- rownames(MM_rfemale)
MM_rfemale$Module <- consensusMods$colors
write.table(MM_rfemale, "Consensus Modules FEMALES RW Module Membership.txt", sep = "\t", quote = FALSE, row.names = FALSE)

MM_rmale <- as.data.frame(moduleMembership$rmdata$data$bicor)
colnames(MM_rmale) <- gsub(pattern = "ME", replacement = "", x = colnames(MM_rmale), fixed = TRUE)
MM_rmale$Probe <- rownames(MM_rmale)
MM_rmale$Module <- consensusMods$colors
write.table(MM_rmale, "Consensus Modules MALES CLAMS Module Membership.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
table(colnames(cov_wtf) == colnames(cov_wtm)) # All TRUE

rownames(cov_wtf) <- cov_wtf$SampleID
...
moduleTraitCorP = cor(consensusMEs$cfdata$data, pheno$cf$data, method = "pearson", use = "p")
...
pdf("Consensus Modules Correlation Plot CLAMS Female Pearson2.pdf", width = 11, height = 15)
...
pdf("Consensus Modules CLAMS Female Pearson Correlation Plot2.pdf", width = 11, height = 15)
sizeGrWindow(width = 11, height = 15)
par(mar = c(9, 8, 1, 2))
...
sizeGrWindow(width = 11, height = 15)
par(mar = c(9, 8, 1, 2))
labeledHeatmap(Matrix = moduleTraitCorS, xLabels = colnames(moduleTraitCorS), yLabels = rownames(moduleTraitCorS), 
               ySymbols = gsub("ME", "", rownames(moduleTraitCorS)), colorLabels = FALSE, colors = blueWhiteRed(50), 
               textMatrix = star, setStdMargins = TRUE, cex.text = 2, textAdj = c(0.5, 0.8), 
